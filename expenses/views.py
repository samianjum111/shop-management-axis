...
def expense_dashboard(request, **kwargs):
    ...
    totals = expenses.aggregate(
        ...
        total_given=Sum('amount', filter=Q(is_credit=True, is_repaid=False)),
        total_taken=Sum('amount', filter=Q(category='taken_loan', is_repaid=False)),
    )
    total_expenses = totals['total_expenses'] or 0
    total_given = totals['total_given'] or 0
    total_taken = totals['total_taken'] or 0
