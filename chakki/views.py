...
def chakki_home(request, **kwargs):
    ...
    total_revenue = all_orders.filter(status='completed').aggregate(Sum('total_amount'))['total_amount__sum'] or Decimal('0')

    # Daily order counts for last 7 days (using all orders)
...
def add_order(request, **kwargs):
    ...
    if select:
        ...
        for c in customers:
            ...
            c.total_pending = get_customer_total_pending(c)
    ...
    if customer_id:
        ...
        customer.total_pending = get_customer_total_pending(customer)
...
@login_required
def add_pending_amount(request, customer_id, **kwargs):
    """Add a pending (udhaar) amount to a customer via an expense entry."""
    customer = get_object_or_404(ChakkiCustomer, id=customer_id, tenant=request.tenant)
    if request.method == 'POST':
        ...
        # Create an expense as a given loan (udhaar)
        Expense.objects.create(
            tenant=request.tenant,
            title=f"Udhaar for {customer.name}",
            amount=amount,
            category='given_loan',
            person_name=customer.name,
            phone=customer.phone,
            ...
            is_credit=True,
            is_repaid=False,
            notes=f"Added from customer profile by {request.user.username}"
        )
        messages.success(request, f"₹{amount} added to {customer.name}'s pending balance.")
        return redirect('customer_profile', schema_name=request.tenant.schema_name, customer_id=customer.id)
    # GET: show a simple form (or redirect to profile)
    return redirect('customer_profile', schema_name=request.tenant.schema_name, customer_id=customer.id)


@login_required
def edit_customer(request, customer_id, **kwargs):
    ...
    if request.method == 'POST':
        ...
        if not name:
            ...
            return redirect('edit_customer', schema_name=request.tenant.schema_name, customer_id=customer.id)
        customer.name = name
        customer.phone = phone
        customer.address = address
        customer.save()
        messages.success(request, f"Customer details updated.")
        return redirect('customer_profile', schema_name=request.tenant.schema_name, customer_id=customer.id)
    # GET: render edit form
    context = {'customer': customer, 'tenant': request.tenant}
    template = 'mobile/edit_customer.html' if request.mobile else 'desktop/edit_customer.html'
    return render(request, template, context)
